                ...
                for (let i = 0; i < sorted.length; i++) {
                    lb_div.appendChild(
                        createLeaderboardCard(
                            master_data[sorted[i]],
                            sorted[i],
                            master_data[sorted[i]][current_year]['POSITION'],
                            years[0]
                        )
                    );
                }
...
function createLeaderboardCard(player_data, player_id, position, year) {
    ...
    if (player_data[year]['TOTAL'] < player_data[year]['PAR']) {
        ...
        score_span.innerHTML =
            '-' + (player_data[year]['PAR'] - player_data[year]['TOTAL']);
    } else if (player_data[year]['TOTAL'] > player_data[year]['PAR']) {
        score_span.innerHTML =
            '+' + (player_data[year]['TOTAL'] - player_data[year]['PAR']);
    ...
    if (player_data[year]['MISSED_CUT']) holes_played += ' (MC)';
...
function createScrollButtons(max_players, lb_div) {
    ...
    arrow.innerHTML = `<i class="arrow right"></i>`;
